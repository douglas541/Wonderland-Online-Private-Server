...
using PServer.Core.Network;
using PServer.Core.Network.DataExt;

namespace PServer.Core.Network
{
    public class cListener
    {
        ...
        private void ListenForNewClient()
        {
            ...
            try
            {
                // Must pair BeginAcceptSocket with EndAcceptSocket (BeginAcceptTcpClient will not work with EndAcceptSocket on .NET 8+)
                this.listener.BeginAcceptSocket(this.AcceptClient, null);
            }
            catch { }
        }
        ...
        private void AcceptClient(IAsyncResult ar)
        {
            ...
            // If Limit > 0, wait until there's room for a new player
            while (globals.Limit > 0 && globals.gCharacterManager.GetCount() >= globals.Limit)
            {
